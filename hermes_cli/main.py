...
Usage:
    ...
    hermes uninstall           Uninstall CNCNS Agent
...
def cmd_whatsapp(args):
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            ...
            print()
            print("  Tip: Agent responses are prefixed with '⚕ CNCNS Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
...
def cmd_uninstall(args):
    """Uninstall CNCNS Agent (or just the Chat GUI with --gui)."""
...
def cmd_update(args):
    """Update CNCNS Agent to the latest version.
    ...
    if is_managed():
        managed_error("update CNCNS Agent")
        return
...
def cmd_acp(args):
    """Launch CNCNS Agent as an ACP server."""
    try:
