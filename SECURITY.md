# CNCNS Agent Security Policy

This document describes CNCNS Agent's trust model, names the one
...
security vulnerabilities. **CNCNS Agent does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted CNCNS Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in CNCNS Agent's documentation
...
CNCNS Agent supports two OS-level isolation postures. They address
...
CNCNS Agent supports this in two ways:

- **CNCNS Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, CNCNS Agent's in-process heuristics
...
CNCNS Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in CNCNS Agent
itself. Bugs in CNCNS Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   CNCNS Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, CNCNS Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  CNCNS Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not CNCNS Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in CNCNS Agent. Bugs in Hermes
...
- CNCNS Agent includes supply-chain guards for MCP server
